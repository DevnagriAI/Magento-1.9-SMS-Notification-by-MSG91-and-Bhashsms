...
class Fourtek_SmsNotification_Model_Observer
{
		...
		/***************sms end*********************/       
			...
			public function cancleafter(Varien_Event_Observer $observer)
			{
				...
				$cancelsms=Mage::getStoreConfig('smanotification/smstextarea/cancelsms');
				$cancelsms = str_replace('{{customer_name}}', $custname, $cancelsms);
				$cancelsms = str_replace('{{order_id}}', $orderId, $cancelsms);
				$cancelsms = str_replace('{{order_amt}}', $orderAmt, $cancelsms);
